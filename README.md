...
This repository includes implementations for controlling DJI Tello drones using YOLOv9-based gesture recognition combined with MediaPipe for face tracking and face recognition.
...
A streamlined implementation for basic drone control:
...
The system recognizes the following hand gestures for drone control:
...
- **Automatic Authentication**: Face recognition runs every 10 seconds
...
2. **Prepare Face Recognition**:
   - Ensure `core_embeddings.pkl` contains face embeddings
...
- **Hardware**: DJI Tello drone, webcam (for face recognition)
- **Software**: Python 3.7+, CUDA-capable GPU (recommended)
- **Model**: Pre-trained YOLOv9 model (`v9_c_best.pt`)
- **Assets**: UI images in `./assets/` directory
